...
function konus (element)
	...
	elseif (ped == "Ali Soyyiğit") then
		triggerEvent("shop.render",localPlayer)
    ...
    elseif (ped=="Lara Sultan") then 
       	triggerServerEvent('clothing:list', getResourceRootElement(getResourceFromName("un_clothes")))								
	...
	elseif (ped=="Mike Moore") then
		...
		exports.un_hud:sendBottomNotification(getLocalPlayer(),(ped or "NPC").." diyor ki:", "'Sana yardımcı olabileceğim bir konu yok, üzgünüm.'")
	end
end
