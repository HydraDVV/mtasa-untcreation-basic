...
function showSettingsWindow(tab)
	...
	if getResourceFromName("un_carradio") then
	...
	if getResourceFromName("un_logs") then
	...
	if getResourceFromName("un_logs") and getResourceFromName("un_phone") then
	...
	if getResourceFromName("un_chat") then
	...
	if getResourceFromName("un_chat") and guiCheckBoxGetSelected(GUIEditor_Checkbox.graphic_chatbub) then
	...
	if getResourceFromName("un_hud") then
	...
	if getResourceFromName("un_shader_radar") then
	...
	if getResourceFromName("un_shader_water") then
	...
	if getResourceFromName("un_shader_car_paint") then
	...
	if getResourceFromName('un_vehicle') then
	...
	if getResourceFromName("un_realism") then
	...
	posY = posY + lineH
	local dyn_light = exports.un_global:isResourceRunning('un_dynamic_lighting') and true or false
	...
	local xmas = exports.un_global:isResourceRunning('un_xmas') and true or false
	if xmas then
		posY = posY + lineH
	...
	if exports.un_integration:isPlayerSupporter(getLocalPlayer()) or exports.un_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		posY = posY + lineH
	...
	if exports.un_integration:isPlayerStaff(localPlayer) then
		posY = posY + lineH
	...
	if getResourceFromName("un_description") then
...
function applyGameSettings(name, value)
	if name and value then
		...
		elseif name == "hide_hud" then
			...
			elseif exports.un_global:hasItem(localPlayer, 111) then
				setPlayerHudComponentVisible("radar", true)
			end
		elseif name == "groundsnow" then
			local groundsnow = exports.un_global:isResourceRunning('un_shader_snow_ground') and true or false
			if groundsnow then
				if value == "0" then
