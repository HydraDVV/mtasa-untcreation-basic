...
function onGuiChange()
    ...
    if now == "Char-Reg.Name" then
        ...
        if utfSub(guiGetText(gui), #guiGetText(gui), #guiGetText(gui)) == "_" then
            ...
            guiEditSetCaretIndex(gui, #guiGetText(gui))
            exports.un_notification:create(thePlayer, "'_' Yerine boşluk kullanabilirsiniz.", "error")
        end
...
function stopCharacterRegistration()
    ...
    skinPed:destroy()
    exports.un_notification:create("Karakteriniz başarıyla oluşturuldu.", "success")
...
addEventHandler("onClientResourceStop", resourceRoot,
    function()
        exports["un_blur"]:removeBlur("boxBlur")
    end
)
...
function drawnText()
    ...
    if tStart then
        ...
        if progress >= 1 then
            exports.un_notification:create("Devam etmek için 'ENTER' tuşunu kullanın!", "info")
            ...
            setTimer(
                function()
    				...
    				prew = exports['un_object_preview']:createObjectPreview(skinPed, 0, 0, 175, x, y, w, h)
    				skinPed:setDimension(localPlayer:getDimension())
    				skinPed:setInterior(localPlayer:getInterior())
    ...
    local font = exports['un_fonts']:getFont("DeansGateB", 12)
...
function startCharRegPanel()
    ...
    font = exports['un_fonts']:getFont("RobotoB", 10)
...
function findSwear(msg)
    local color = exports['un_coloration']:getServerColor("red", true)
    local color2 = exports['un_coloration']:getServerColor(nil, true)
    local white = "#ffffff"
    local without = msg
    local strF = without
...
function stopCharRegPanel()
    ...
    if height < 150 or height > 220  then
        exports.un_notification:create("Boyunuz 150 ile 220 cm aralığında olmalıdır.", "error")
        return
    end

    if weight < 45 or weight > 120  then
        exports.un_notification:create("Kilonuz 45 ile 120 aralığında olmalıdır.", "error")
        return
    end

    if age < 16 or age > 80 then
        exports.un_notification:create("Yaşınız 16 ile 80 aralığında olmalıdır.", "error")
        return
    end
    ...
    while true do
        ...
        if a then
            if #a <= 2 then
                exports.un_notification:create("Girdiğiniz isim uygun değil.", "error")
                return
            end
            count = count + 1
            a = a .. "_"
    ...
    if count < 3 then
        exports.un_notification:create("İkiden fazla isim kullanamazsınız.", "error")
        return
    end

    if name ~= findSwear(name) then
        exports.un_notification:create("İsminizde yasaklı kelimeler bulunuyor.", "error")
        return
    end
...
addEventHandler("receiveNameRegisterable", root,
    function(a, b)
        if a then
            ...
            exports.un_notification:create("Böyle bir karakter bulunuyor!", "error")
            return
        end
    end
)
...
function drawnCharRegPanel()
	...
	dxDrawRectangle(0, 0, zX, zY, tocolor(0, 0, 0, 220), false);
    ...
    local _w, _h = w, h
	alpha = 255
	rx,rg,rb = 219, 217, 219

    local w, h = 204, 34
    local x, y = sx/2 - w - 60, sy/2 - h/2 + 15
    dxDrawRoundedRectangle(x,y,w,h,tocolor(rx,rg,rb, alphaq), { 1.0, 1.0, 1.0, 1.0 }, false);

    local bName = "Char-Reg.Height"
    UpdatePos(bName, {x,y,w,h})
    UpdateAlpha(bName, tocolor(50, 48, 50, alpha))
    ...
    if isInSlot(x,y,w,h) or cSelected == 2 then
        ...
        local r,g,b = exports['un_coloration']:getServerColor("white")
...
function drawnLoadingScreen()
    local font = exports['un_fonts']:getFont("RobotoB", 8)
    local font2 = exports['un_fonts']:getFont("Roboto", 8)
