function spawnPl(sourceElement, SkinID, x,y,z, rot, dim,int, Health, Armor, fightStyle, walkStyle, deathReasons, stats)
    ...
    triggerClientEvent(sourceElement, "cameraSpawn", sourceElement, Health)
end
addEvent("spawnPl", true)
addEventHandler("spawnPl", root, spawnPl)


function loginYazdir(sourceElement)
...
addEvent("login.yazdir", true)
addEventHandler("login.yazdir", root, loginYazdir)

function unFreeze(sourceElement, hp)
    sourceElement:setAlpha(50)
    ...
    if hp > 0 then
        exports.un_notification:create(sourceElement,"Giriş başarılı, iyi oyunlar dileriz!", "success")
...
function interactLogin(sourceElement, username, password)  
    ...
    if not isValidAccount[username] then
        exports.un_notification:create(sourceElement,"Bu kullanıcı adı ("..username..") kayıtlı değil.", "error")
        return
    end

    if isLogged[username] then
        exports.un_notification:create(sourceElement,"Bu kullanıcı adı ("..username..") zaten kullanımda.", "error")
        return
    end
    ...
    if string.lower(password) ~= string.lower(realPassword) then
        ...
        if string.lower(realPassword) ~= string.lower(hashedPassword2) then
            exports.un_notification:create(sourceElement,"Yanlış parola!", "error")
            return
        end
    end
    ...
    if banned then
        ...
        kickPlayer(sourceElement, "Rendszer", "Sunucudan yasaklandınız!")
    ...
    if string.lower(serial) ~= string.lower(mtaserial) then
        if tostring(serial) == "0" or tonumber(serial) == 0 then -- Serial váltás
            if not isSerialAttachedToAccount[mtaserial] then
                ...
                exports.un_notification:create(sourceElement,"Serialiniz başka bir kullanıcı adına kayıtlı! ("..tostring(isSerialAttachedToAccount[mtaserial])..")!", "error")
                return
            end
        else
            exports.un_notification:create(sourceElement,"Serial numaranız ("..mtaserial..") bu kullanıcı adıyla ilişkili değil.", "error")
            return
        end
    end
    ...
    if match and match[1] then
        if accounts[match[1]] then
            ...
            if serial ~= sourceElement:getData("mtaserial") then
                exports.un_notification:create(sourceElement,"Bu hesap ("..match[1]..") serialinize ait değil.", "error")
                return
            end
        end
    end
...
function destroyCode(e, val, ignore)
    if codes[e] then
        codes[e] = nil
        if not ignore then
            exports.un_notification:create(sourceElement,"15 dakika geçtiği için yeni bir kod istemeniz gerekecek!", "error")
...
function forgetPassSend(sourceElement, val)
    ...
    if lastClickTick + minutes * 60 * 1000 > getTickCount() then
        exports.un_notification:create(sourceElement,""..minutes.." Her dakika bir şifre hatırlatma e-postası gönderebilirsiniz!", "error")
        return
    end
    ...
    callRemote("http://mtaapi.bluemta.com/passwordRecovery.php?userName="..accName.."&userEmail="..accEmail.."&recoveryCode="..code, function() end)
    ...
    exports.un_notification:create(sourceElement,"Kod E-Posta adresinize gönderildi, "..minutes.." dakikanız var.", "info")
end
