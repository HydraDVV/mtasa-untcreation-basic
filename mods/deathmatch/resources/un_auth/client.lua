...
addEventHandler("accounts:login:attempt", getRootElement(),
	function (statusCode, additionalData, pChars)
		if (statusCode == 0) then
			...
			exports.un_notification:create("Devam etmek için 'ENTER' / Yeni bir karakter oluşturmak için 'SPACE' tuşlarını kullanın!", "info")
...
addEventHandler("register", root,
	function(username, password)
		...
		if access then
			exports.un_notification:create("Başarıyla "..username.." isimli hesabın sahibi oldun.", "success")
	...
	if state then
		...
		if not renderData.charSelectInterpolation then
			...
			elseif key == "space" and not renderData.charGotSelected and not (#getElementData(getLocalPlayer(), "account:characters") >= getElementData(getLocalPlayer(), "account:charLimit")) then
                ...
                outputChatBox("UNT: #f9f9f9Karakter Limitine Sahip Değilsiniz /market Üzerinden Alabilirsiniz.", 255, 0, 0, true)			
			end
		end
	end
end
