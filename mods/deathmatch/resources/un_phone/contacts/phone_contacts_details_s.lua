...
function updateContactDetails(id, name, number, email, address, favo)
	...
	if dbExec(mysql:getConnection(),"UPDATE `phone_contacts` SET `entryName`='"..exports.un_global:toSQL(name).."', `entryNumber`='"..exports.un_global:toSQL(number).."', `entryEmail`='"..exports.un_global:toSQL(email).."', `entryAddress`='"..exports.un_global:toSQL(address).."', `entryFavorited`='"..exports.un_global:toSQL(favo).."' WHERE `id`='"..exports.un_global:toSQL(id).."' ") then
...
function deleteContact(id)
	...
	if dbExec(mysql:getConnection(),"DELETE FROM `phone_contacts` WHERE `id`='"..exports.un_global:toSQL(id).."' ") then
