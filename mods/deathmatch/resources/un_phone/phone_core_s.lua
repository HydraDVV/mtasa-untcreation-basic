...
local connection = exports["un_mysql"]:getConnection()
addEventHandler("onResourceStart", root, 
    function(startedRes)
        if getResourceName(startedRes) == "un_mysql" then
            connection = exports["un_mysql"]:getConnection()
...
function applyPhone(string, popOutOnPhoneCall)
    ...
    items = exports["un_items"]:getItems(source)
    phoneName = "iPhone 7s"
    for index, row in ipairs(items) do
        if row[1] == 2 then
            phoneName = exports["un_items"]:getItemName(2, row[2], row[5])
        end
    end
    ...
    if string == "phone_in" then
        ...
        if getElementData(source, "phone_anim") ~= "0" then
            ...
            exports.un_bone_attach:attach(phoneO[source], source, 12, -0.05, 0.02, 0.02, 20, -90, -10)
        ...
        if getElementData(source, "cellphoneGUIStateSynced") ~= 1 then
            exports.un_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
        end
        exports.un_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
    elseif string == "phone_talk" then
        if getElementData(source, "phone_anim") ~= "0" then
            ...
            setElementDimension(phoneO[source], getElementDimension(source))
            setElementInterior(phoneO[source], getElementInterior(source))
            exports.un_bone_attach:attach(phoneO[element], source, 12, -0.03, 0.02, 0.02, 20, -90, -10)
    ...
    elseif string == "phone_out" then
        ...
        exports.un_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", nil , true)
        if isElement(phoneO[source]) then
...
function destroyPhone(element)
    if canPlayerCall(element) then
        exports.un_global:removeAnimation(element) 
    end
    if isElement(phoneO[element]) then
        exports.un_bone_attach:detach(phoneO[element])
...
function callSomeone(thePlayer, commandName, phoneNumber, withNumber)
    ...
    if (logged==1) then
        ...
        if publicphone or exports.un_global:hasItem(thePlayer, 2) then
            ...
            elseif withNumber and withNumber > 10 then
                if exports.un_global:hasItem(thePlayer, 2, tonumber(withNumber))  then
            ...
            if callerphoneIsTurnedOn == 0 then
                ...
                elseif getElementData(thePlayer, "injuriedanimation") then
                    ...
                    if isNumberAHotline(phoneNumber) then
                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "calling", phoneNumber, false)
                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "callingwith", outboundPhoneNumber, false)    
                    ...
                    -- Otherwise find a fool to answer it
                        ...
                        if bankMoney >= 1 then
                            if not exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "bankmoney", tonumber(bankMoney) - 1, false) then
                            ...
                            function(qh, thePlayer)
                                ...
                                if rows > 0 then
                                    for index, phoneSettings in ipairs(res) do
                                        ...
                                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "call.col", publicphone, false)
                                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "calling", tonumber(phoneNumber), false)
                                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "called", true, false)
                                        exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "callingwith", outboundPhoneNumber, false)
                                        exports.un_anticheat:changeProtectedElementDataEx(foundElement, "calling", tonumber(outboundPhoneNumber), false)
                                        exports.un_anticheat:changeProtectedElementDataEx(foundElement, "called", nil, false)
                                        ...
                                        if not reconning then 
                                            if calledphoneRingTone ~= 0 then
                                                for _,nearbyPlayer in ipairs(exports.un_global:getNearbyElements(foundElement, "player"), 10) do
...
function makeFCall(thePlayer, commandName, phoneNumber)
    ...
    if fPhone and factionPhone then
        ...
        if not (phoneNumber) then
            ...
            local hasCellphone, itemKey, itemValue, itemID = exports.un_global:hasItem(thePlayer, 2)
            if itemValue then
    ...
    for _, phoneNumber in ipairs(phoneNumbers) do
        ...
        if found and foundElement and isElement(foundElement) then
            ...
            if (phoneState==0) then
                exports.un_anticheat:changeProtectedElementDataEx(foundElement, "calling", nil, false)
                exports.un_anticheat:changeProtectedElementDataEx(foundElement, "called", nil, false)
                exports.un_anticheat:changeProtectedElementDataEx(foundElement, "call.col", nil, false)
            end
        end
    end
end
...
function phoneBook(thePlayer, commandName, partialNick)
    ...
    if (logged==1) then
        if (exports.un_global:hasItem(thePlayer, 7)) then
...
function togglePhone(thePlayer, commandName, phoneNumber)
    ...
    if logged == 1 then
        if not phoneNumber then
            local foundPhone,_,foundPhoneNumber = exports.un_global:hasItem(thePlayer, 2)
        ...
        elseif tonumber(phoneNumber) < 10 then
            ...
            if not (exports.un_global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
        ...
        for key, value in ipairs(getElementsByType("player")) do
            ...
            if (tonumber(logged)==1) then
                if phoneNumber >= 10000 then                 
                    local foundPhone,_,foundPhoneNumber = exports.un_global:hasItem(value, 2, tonumber(phoneNumber))
                    if foundPhone then
                        return true, value
                    end
                elseif phoneNumber >= 1000 and exports.un_global:hasItem(value, 2) then -- 4 digit number for factions (2 digits for faction, 2 digits for specific player)
...
function fetchFirstPhoneNumber(target)
    local foundPhone,_,foundPhoneNumber = exports.un_global:hasItem(target, 2, tonumber(phoneNumber))
    return foundPhoneNumber
end

function setEDX(thePlayer, index, newvalue, sync, nosyncatall)
    return exports.un_anticheat:changeProtectedElementDataEx(thePlayer, index, newvalue, sync, nosyncatall)
end
...
function cleanUpOnePlayer(player)
    ...
    exports.un_anticheat:changeProtectedElementDataEx(player, "cellphoneGUIStateSynced", nil, true)
end
...
function findToNumber(player,number)
    ...
    for k,v in ipairs(getElementsByType("player")) do
        if getElementData(v,"loggedin") == 1 then
            if exports["un_global"]:hasItem(v,2,number) then
                found = found + 1
                if found == 1 then
