...
function SAN_newAdvert(message, sourceName, sourceElement, theCost, outboundPhoneNumber)
	...
	exports.un_global:sendMessageToSupporters("[ADVERT] ID#"..advertID .. " ("..advertMsg..") created by "..sourceName..".")
	...
	for adIndex, adTable in ipairs (ads) do
		...
		elseif adTable[ AD_FROZEN ] then
			...
			if (currentTime - adTable [ AD_EDITTIME ]) > 5 then
				table.insert(removeIDs, adIndex)
				exports.un_global:sendMessageToSupporters("[ADS] Removing ad "..adIndex.." due being 10 minutes frozen. (`"..adTable[AD_MESSAGE].."`, `"..adTable[AD_SENDERNAME].."`)")
			end
		elseif ( not didAirSomething ) and not (adTable[AD_EDITTIME] == adTable[AD_CREATIONTIME] ) then
			...
			exports.un_logs:logMessage("ADVERT: " .. adTable[AD_MESSAGE] .. " ((".. adTable[AD_SENDERNAME].." ))", 2)
			for key, value in ipairs(exports.un_pool:getPoolElementsByType("player")) do
				if getElementData(value, "loggedin")==1 then
					
						if exports.un_integration:isPlayerSupporter(value) or exports.un_integration:isPlayerTrialAdmin(value) then
...
function SAN_freezeAdvert(thePlayer, commandHandler, advertID)
	if exports.un_integration:isPlayerTrialAdmin(thePlayer) or exports.un_integration:isPlayerSupporter(thePlayer) then
		...
		exports.un_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." froze advert "..advertID .. ".")
	end
end
...
function SAN_unfreezeAdvert(thePlayer, commandHandler, advertID)
	if exports.un_integration:isPlayerTrialAdmin(thePlayer) or exports.un_integration:isPlayerSupporter(thePlayer) then
		...
		ads[advertID][AD_EDITTIME] = getRealTime().timestamp
		exports.un_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." unfroze advert "..advertID .. ".")
...
function SAN_deleteAdvert(thePlayer, commandHandler, advertID)
	if exports.un_integration:isPlayerTrialAdmin(thePlayer) or exports.un_integration:isPlayerSupporter(thePlayer) then
		...
		exports.un_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." marked advert "..advertID .. " as aired.")
	end
end
