...
function openEmailWindow()
    ...
    if not (wEmail) then
        ...
        local teamID = exports.un_factions:getCurrentFactionDuty(getLocalPlayer())
        if (teamID==1) then
...
function show_inbox(inbox_table, accountName)
    if not (wEmail) then
        ...
        --accoun_tab = guiCreateTab("Account Settings",email_tab_panel)
        ...
        -- Change Password
        old_password_label = guiCreateLabel(0.1,0.1,0.3,0.05,"Old Password",true,accoun_tab)
        old_password_input = guiCreateEdit(0.4,0.1,0.3,0.05,"",true,accoun_tab)
        guiEditSetMasked(old_password_input,true)
        new_password_label = guiCreateLabel(0.1,0.2,0.3,0.05,"New Password",true,accoun_tab)
        new_password_input = guiCreateEdit(0.4,0.2,0.3,0.05,"",true,accoun_tab)
        guiEditSetMasked(new_password_input,true)
        confirm_password_label = guiCreateLabel(0.1,0.3,0.3,0.05,"Confirm New Password",true,accoun_tab)
        confirm_password_input = guiCreateEdit(0.4,0.3,0.3,0.05,"",true,accoun_tab)
        guiEditSetMasked(confirm_password_input,true)
        --save button
        save_button = guiCreateButton(0.4,0.4,0.2,0.08,"Save",true,accoun_tab)
        -- Clear Button

        -- Delete Account
        delete_accoun_label = guiCreateLabel(0.1,0.5,0.8,0.3,"To delete this email account enter your password below and\
                                                            press the delete account button.\
                                                            \
                                                            Deleting an account is permenant and can not be undone.",true,accoun_tab)
        delete_accoun_input = guiCreateEdit(0.1,0.7,0.3,0.05,"",true,accoun_tab)
        guiEditSetMasked(delete_accoun_input,true)
        --button
        delete_accoun_button = guiCreateButton(0.4,0.8,0.2,0.08,"Delete Account",true,accoun_tab)]]
