...
function updateNametagColor(thePlayer)
	...
	elseif exports.un_integration:isPlayerTrialAdmin(thePlayer) and getElementData(thePlayer, "duty_admin") == 1 and getElementData(thePlayer, "hiddenadmin") == 0 then -- Admin on duty
		setPlayerNametagColor(thePlayer,  242, 9, 9)
	elseif exports.un_integration:isPlayerSupporter(thePlayer) and (getElementData(thePlayer, "duty_supporter") == 1) and getElementData(thePlayer, "hiddenadmin") == 0 then 
		setPlayerNametagColor(thePlayer, 70, 200, 30)
	else
		setPlayerNametagColor(thePlayer, 255, 255, 255)
	end
	for index, value in pairs(exports["un_items"]:getBadges()) do
