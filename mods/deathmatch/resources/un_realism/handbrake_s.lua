...
function toggleHandbrake( player, vehicle, forceOnGround, commandName )
	...
	if (handbrake == 0) then
		if getVehicleType(vehicle) == 'BMX' or getVehicleType(vehicle) == 'Bike' then
			...
			elseif math.floor(exports.un_global:getVehicleVelocity(vehicle)) > 2 then
				outputChatBox("Bu sürüş sırasında işe yaramaz...", player, 255, 0, 0)
			else
				exports.un_anticheat:changeProtectedElementDataEx(vehicle, "handbrake", 1, true)
		...
		exports.un_anticheat:changeProtectedElementDataEx(vehicle, "handbrake", 0, true)
