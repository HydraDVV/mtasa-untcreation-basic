...
local font = exports.un_fonts:getFont("Modern", 18)
local fuel = 0
...
function drawSpeedo()
	...
	if (vehicle) then
		speed = exports.un_global:getVehicleVelocity(vehicle, getLocalPlayer())
		local x = width
		local y = height
...
function drawFuel()
	...
	if (vehicle) then
		local x = width
		local y = height
		local FuelPer = (fuel/exports["un_vehicle_fuel"]:getMaxFuel(vehicle))*100
		local ax, ay = x - 180, y/2
		if FuelPer > 100 then
			FuelPer = fuel 
		end
