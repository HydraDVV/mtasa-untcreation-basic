	...
	if getPedOccupiedVehicle(thePlayer) then
		...
		if (getVehicleType(theVehicle) == "BMX" or getVehicleType(theVehicle) == "Bike") or (noBelt[getElementModel(theVehicle)] and getVehicleOccupant(theVehicle, 0) ~= thePlayer) then
			...
			if 	(getElementData(thePlayer, "seatbelt") == true) then
				exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "seatbelt", false, true)
				--outputChatBox("Emniyet kemerini çıkarttın.", thePlayer, 255, 0, 0)
				exports.un_global:sendLocalMeAction(thePlayer, "kemerin kilidini açarak kemeri çıkartır.")
			else
				exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "seatbelt", true, true)
				--outputChatBox("Emniyet kemerini taktın.", thePlayer, 0, 255, 0)
				exports.un_global:sendLocalMeAction(thePlayer, "emniyet kemerinin ucunu çekerek kilitler.")
			end
		end
	end
end
