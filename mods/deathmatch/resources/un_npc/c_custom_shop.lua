...
function addItemToShop(source, item, slot, worldItem, npcID, shopElement )
	...
	if tonumber(item[1]) == 16 then --clothes
		local value = exports.un_global:explode(":",tostring(item[2]))
		local skinid = tonumber(value[1]) or 1
		itemURL = ":un_auth/img/" .. ("%03d"):format( skinid or 1 ) .. ".png"
	...
	local itemName = exports["un_items"]:getItemName( tonumber(item[1]), tostring(item[2]) ) or ""
	local itemValue = ""
	if not exports["un_items"]:getItemHideItemValue(tonumber(item[1])) then
		itemValue = exports["un_items"]:getItemValue( tonumber(item[1]), tostring(item[2]) ) or ""
	end
	...
	local mDesc = guiCreateMemo(147,99,280,56,exports["un_items"]:getItemDescription( tonumber(item[1]), tostring(item[2]) ) or "",false,wAddingItemsToShop)
...
function customShopBuy(products, proID, shopElement)
	...
	local ePrice = guiCreateLabel(189,62,238,20,exports.un_global:formatMoney(itemPrice),false,wCustomShopBuy)
