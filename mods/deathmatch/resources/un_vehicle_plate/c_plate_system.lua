...
function getPaperFromTony(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then --if it's a right-click on a object
        ...
        if(pedName == "Tony Johnston") then
            rcMenu = exports.un_rightclick:create(pedName)
            showCursor(true)
            local row = exports.un_rightclick:addRow("Buy DMV transaction paper - 100$")
            addEventHandler("onClientGUIClick", row,  function (button, state)
            	if exports.un_global:hasMoney(localPlayer, 100) and getElementData(localPlayer, "loggedin") == 1 then
            ...
            local row2 = exports.un_rightclick:addRow("Close")
            addEventHandler("onClientGUIClick", row2,  function (button, state)
                exports.un_rightclick:destroy(rcMenu)
                showCursor(false)
            end, false)
        end
    end
...
function cBeginGUI()
	...
	greetingWindow = guiCreateStaticImage(x, y, width, height, ":un_resources/window_body.png", false)
...
function PlateWindow(vehicleList)
	...
	for i,r in ipairs(vehicleList) do
		...
		guiGridListSetItemText(vehlist, row, ov, exports.un_global:getVehicleName(v) , false, false) --tostring(getVehicleNameFromModel(vm))
...
function editPlateWindow()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		efinalWindow = guiCreateStaticImage(x, y, width, height, ":un_resources/window_body.png", false)
...
function updateRegistration()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		if exports.un_global:hasMoney(localPlayer,  state == "Unregistered" and 175 or 50 ) then
...
function insuranceDetails()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		local insuranceFactionName = exports.un_factions:getFactionName(tonumber(data.insurancefaction))
		...
		string = string.."Premium: $"..exports.un_global:formatMoney(tonumber(data.premium)).."\n"
