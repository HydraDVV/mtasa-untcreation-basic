...
spam[source] = tonumber(spam[source] or 0) + 1
		if spam[source] >= 9 then
			...
			outputChatBox('[UNT]#D0D0D0 Bu kadar sık komut kullanımı yapmayınız.',source, 195,184,116,true)
			exports.un_anticheat:changeProtectedElementDataEx(source, "komutkullanimikapat", true)
		...
		zaman[source] = setTimer(	function (source)
			...
			if isElement(source) and getElementData(source, "komutkullanimikapat") == true or getElementData(source, "komutkullanimikapat") then
				exports.un_anticheat:changeProtectedElementDataEx(source, "komutkullanimikapat", false)
