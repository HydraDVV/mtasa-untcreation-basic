...
function startSystem()
	for key, group in ipairs(tollPorts) do
		...
		for dataKey, dataGroup in ipairs (group.data) do
			...
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "talk",1, true)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "name", pedName:gsub("_", " "), true)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "ped:type", "toll", true)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "ped:tollped",true, true)

			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang1" , 1, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang1skill", 100, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang2" , 2, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang2skill", 100, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.current", 1, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:object", tollElements[key][dataKey]["object"], false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:data", dataGroup, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:busy", false, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:state", false, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:name", group.name, false)
			exports.un_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:key", key, false)
			...
			exports.un_anticheat:changeProtectedElementDataEx(col, "toll:ped", tollElements[key][dataKey]["ped"], false)
			...
			exports.un_anticheat:changeProtectedElementDataEx(col2, "toll:ped", tollElements[key][dataKey]["ped"], false)
...
function triggerGate(gateKeeperPed, earlyOpenZone) -- OPEN GATE
	...
	exports.un_anticheat:changeProtectedElementDataEx(gateKeeperPed, "toll:busy", true, false)
	...
	exports.un_anticheat:changeProtectedElementDataEx(gateKeeperPed, "toll:opened", true, false)
	...
	if earlyOpenZone then
		...
		exports.un_anticheat:changeProtectedElementDataEx(gateKeeperPed, "earlyOpened", true, false)
		timerEarlyOpen[gateKeeperPed] = setTimer(function()
			exports.un_anticheat:changeProtectedElementDataEx(gateKeeperPed, "earlyOpened", false, false)
...
function processCloseTolls(tollKey, thePed, thePlayer, payByBank) -- CLOSE GATE
	...
	moveObject ( tollObject, gateSpeed, newX, newY, newZ, offsetRX, offsetRY, offsetRZ )
	exports.un_anticheat:changeProtectedElementDataEx(thePed, "toll:opened", false, false)
	exports.un_anticheat:changeProtectedElementDataEx(thePed, "toll:busy", true, false)
...
function startTalkToPed ()
	...
	if (isPedInVehicle(thePlayer)) then
		...
		if (exports['un_vehicle']:isVehicleWindowUp(theVehicle)) then
...
function processOpenTolls(tollKey, thePed, thePlayer, payByBank)
	if not tollElementsLocked[tollKey] or exports.un_global:hasItem(thePlayer, 64) or exports.un_global:hasItem(thePlayer, 65) or exports.un_global:hasItem(thePlayer, 112) then
		if payByBank then
			if not exports.un_global:hasItem(getPedOccupiedVehicle(thePlayer), 118) then
				return -- Has no Toll Pass
			end

			local money = getElementData(thePlayer, "bankmoney") - 12
			if money >= 0 then
				exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "bankmoney", money, false)
			...
			if not exports.un_global:takeMoney(thePlayer, 12) then
				return "Paran yoksa geçemezsin."
			end
		end

		triggerGate(thePed)
		exports.un_global:giveMoney( getTeamFromName("İstanbul Büyükşehir Belediyesi"), 3 )
		exports.un_global:giveMoney( getTeamFromName("İstanbul Emniyet Müdürlüğü"), 2 )
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if factionID ~= 1 and factionID ~= 78 and not exports.un_integration:isPlayerTrialAdmin(thePlayer) then
		return
	end
...
function setConvoState(thePlayer, state)
	exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "toll:convoState", state, false)
end
...
function processMessage(thePed, message, language)
	...
	exports['un_chat']:localIC(thePed, message, language)
end

function processRadio(thePed, message, source)
	local name = getElementData(thePed, "name") or getPlayerName(thePed)
	exports['un_global']:sendLocalText(source, "[English] " .. name .. "'s Radio: "..message, 255, 255, 255)
end
...
function processDoMessage(thePed, message, source)
	local name = getElementData(thePed, "name") or getPlayerName(thePed)
	exports['un_global']:sendLocalText(source, " * " .. message .. " *      ((" .. name:gsub("_", " ") .. "))", 255, 51, 102)
end
...
function resetBusyState(theGate)
	...
	if (isGateBusy) then
		...
		exports.un_anticheat:changeProtectedElementDataEx(theGate, "toll:busy", false, false)
	end
end
	...
	if duty > 0 and getElementDimension( thePlayer ) == 0 then
		...
		for key, value in ipairs( tollElements ) do
			...
			if name then
				...
				for k, v in ipairs(value) do
					if v.ped then
						exports.un_logs:dbLog(thePlayer, 35, v.ped, "TOLL OPEN " .. name)
