...
local bone = exports.un_bone_attach
...
function addWearablePlayer(player, temp_table)
    if tonumber(player:getData("loggedin")) == 1 then
        ...
        bone:attach(createdObject, player, temp_table.bone, temp_table.x, temp_table.y, temp_table.z, temp_table.rx, temp_table.ry, temp_table.rz);
...
addEventHandler("wearable.delete", root,
    function(player, dbid)
        ...
        outputChatBox(exports.un_pool:getServerSyntax(false, "s").."Seçili aksesuar başarıyla silindi.", player, 255, 255, 255, true)
    end
)
...
addEventHandler("wearable.savePositions", root,
    function(player, data)
        ...
        outputChatBox(exports.un_pool:getServerSyntax(false, "s").."Aksesuarınızın pozisyonu başarıyla kaydedildi!", player, 255, 255, 255, true)
        loadPlayerWearables(player);
    end
);
