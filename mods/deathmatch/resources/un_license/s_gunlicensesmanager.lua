...
function syncTable(guntable)
	if type(guntable) == "table" then
		...
		for k,v in ipairs(exports.un_pool:getPoolElementsByType("player")) do
...
function startThis(thePlayer)
	if (exports.un_integration:isPlayerSeniorAdmin(thePlayer)) or (exports.un_global:hasItem(thePlayer, 209) and (exports.factions:isPlayerInFaction(thePlayer, 1) or exports.factions:isPlayerInFaction(thePlayer, 50))) then
...
function revokeElements(name)
	if name then
		exports.un_anticheat:changeProtectedElementDataEx(getPlayerFromName(name), "license.gun", 0, false)
		exports.un_anticheat:changeProtectedElementDataEx(getPlayerFromName(name), "license.gun2", 0, false)
	end
end
...
function changeElement(name, licensetype)
	if name and licensetype and isElement(getPlayerFromName(name)) then
		if getElementData(getPlayerFromName(name), "loggedin") == 1 then
			exports.un_anticheat:changeProtectedElementDataEx(getPlayerFromName(name), "license."..licensetype, 1, false)
		end
	end
end
