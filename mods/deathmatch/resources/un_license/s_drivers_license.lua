...
function giveCarLicense(usingGC)
	...
	exports.un_hud:sendBottomNotification(source, "Department of Motor Vehicles", "Congratulations! You've passed your driving examination!" )
	exports.un_global:giveItem(source, 133, getPlayerName(source):gsub("_"," "))
	...
	if getElementData(source, "owner") == -2 and getElementData(source, "faction") == -1 and getElementModel(source) == 410 then
		if getElementData(player,"license.car") == 3 then
			if getElementData(player, "license.car.cangetin") then
				exports.un_hud:sendBottomNotification(player, "Department of Motor Vehicles", "You can use 'J' to start the engine and /handbrake to release the handbrake." )
			else
				exports.un_hud:sendBottomNotification(player, "Department of Motor Vehicles", "This vehicle is for the Driving Test only, please see the NPC inside first." )
				if not exports["un_integration"]:isPlayerHeadAdmin(player) then
					cancelEvent()
				end
			end
		elseif seat > 0 then
			exports.un_hud:sendBottomNotification(player, "Department of Motor Vehicles", "This vehicle is for the Driving Test only." )
		else
			exports.un_hud:sendBottomNotification(player, "Department of Motor Vehicles", "This vehicle is for the Driving Test only." )
			cancelEvent()
		end
