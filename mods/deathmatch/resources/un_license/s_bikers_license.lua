...
function giveBikeLicense(usingGC)
	...
	exports.un_hud:sendBottomNotification(source, "Department of Motor Vehicles", "Congratulations! You've passed your motorcycle examination!" )
	exports.un_global:giveItem(source, 153, getPlayerName(source):gsub("_"," "))
...
function passTheory()
	...
	exports.un_global:giveItem(source, 90, 1)
end
	...
	if getElementData(source, "owner") == -2 and getElementData(source, "faction") == -1 and getElementModel(source) == 468 then
		if getElementData(player,"license.bike") == 3 then
			if getElementData(player, "license.bike.cangetin") then
				exports.un_hud:sendBottomNotification(player, "Department of Motor Vehicles", "You can use 'J' to start the engine and /kickstand prior to driving." )
			else
