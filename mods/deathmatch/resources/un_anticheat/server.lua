function showSpeedToAdmins(velocity)
	...
	exports.un_global:sendMessageToAdmins("[Possible Speedhack/HandlingHack] " .. getPlayerName(client) .. ": " .. velocity .. "Mph/".. kph .." Kph")
end
...
function showDMToAdmins(kills)
	exports.un_global:sendMessageToAdmins("[Possible DeathMatching] " .. getPlayerName(client) .. ": " .. kills .. " kills in <=2 Minutes.")
end
...
function scanMoneyHacks()
	...
	for key, value in ipairs(players) do
		...
		if (logged==1) then
			if not (exports.un_integration:isPlayerTrialAdmin(value)) then
				
				local money = getPlayerMoney(value)
				local truemoney = exports.un_global:getMoney(value)
				if (money) then
