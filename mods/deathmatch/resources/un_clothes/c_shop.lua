...
function createShopPed()
	...
	setElementDimension(ped, 10)--2193
...
local function getFittingSkins()
	...
	local temp = exports['un_npc']:getFittingSkins()
	...
	for k, v in ipairs(list) do
		...
		if canBuySkin(localPlayer, v) or canEdit(localPlayer) then
			...
			if not only_fitting or fitting_skins[v.skin] then
				...
				guiGridListSetItemText(grid, row, 4, v.price == 0 and 'N/A' or ('$' .. exports.un_global:formatMoney(v.price)), false, false)
			end
		end
	end
end
				...
				if button == 'left' then
					...
					if row == -1 then
						...
						if item then
							...
							guiSetEnabled(buy, item.price == 0 or exports.un_global:hasMoney(localPlayer, item.price))
