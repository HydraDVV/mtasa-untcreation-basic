local fishingList = {
	{name = "Alabalık", price = 350},
	{name = "Sudak Balığı", price = 200},
	{name = "Dağ Alabalığı", price = 200},
	{name = "Deniz Alabalığı", price = 200},
	{name = "Snapper Balığı", price = 200},
	{name = "Cabak Balığı", price = 200},
	{name = "Butter Balığı", price = 200},
	{name = "Ton Balığı", price = 200},
	{name = "Kılıç Balığı", price = 200},
	{name = "Kalamar Balığı", price = 200},
	{name = "Ringa Balığı", price = 350},
	{name = "Cipura Balığı", price = 250},
	{name = "Gökkuşağı Balığı", price = 250},
	{name = "Siyah Cod Balığı", price = 250},
	{name = "Beyaz Seabass Balığı", price = 250},
	{name = "Göl Alabalığı", price = 250},
	{name = "Sazan Balığı", price = 250},
	{name = "Monk Balığı", price = 250},
	{name = "Beyaz Bass Balığı", price = 250},
	{name = "Benekli Bass Balığı", price = 250},
	{name = "Altın Balığı", price = 250},
	{name = "Dere Alabalığı", price = 250},
}
		...
		if isElementWithinColShape(thePlayer, balikCol) then
			if (not getElementData(thePlayer, "balikTutuyor")) then
				...
				if toplamyem > 0 then
					...
					exports.un_global:applyAnimation(thePlayer, "SWORD", "sword_IDLE", -1, false, true, true, false)
					setElementData(thePlayer, "balikTutuyor", true)
					exports.un_global:sendLocalMeAction(thePlayer, "oltasını denize doğru sallar.", false, true)
					...
					setTimer(function(thePlayer) 
						...
						if rastgeleSayi == 2 then
							...
							exports["un_items"]:giveItem(thePlayer, 273, givenFishTable.name)
...
addCommandHandler("balikdurum", 
	function(thePlayer, cmd)
		...
		local toplamBalik = exports["un_items"]:countItems(thePlayer, 273)
	...
	if para >= 1 then
		if isElementWithinColShape(thePlayer, yemCol) then
			...
			elseif toplamyem <= 20 then
				exports.un_global:takeMoney(thePlayer, 1)
...
function balikSat(thePlayer, cmd)
	local countOfPlayerFish = exports["un_items"]:countItems(thePlayer, 273)
	local countPrice = 0
	local playerFishs = exports["un_items"]:getItems(thePlayer)
	...
	if isElementWithinColShape(thePlayer, yemCol) then
		...
		if toplambalik <= 0 then
			...
			exports.un_global:giveMoney(thePlayer, countPrice)
			for i = 0, countOfPlayerFish do
				exports["un_items"]:takeItem(thePlayer, 273)
			end
