...
-- account logged in
--
addEvent( "social:un_auth", true )
addEventHandler( "social:un_auth", root,
	function( accountID )
...
function drawFriends( )
	...
	for _, data in ipairs( { { title = "Çevimiçi", items = online }, { title = "Çevrimdışı", items = offline } }) do
		...
		for key, value in ipairs( data.items ) do
			...
			-- draw name
			if exports.un_integration:isPlayerTrialAdmin(value.player) then
				dxDrawText( value.name, x, y, w, 15, tocolor ( 255, 194, 14, 255 ), 1, font, "left", "top", false, false, true )
			elseif exports.un_integration:isPlayerSupporter(value.player) then
