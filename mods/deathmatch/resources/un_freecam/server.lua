...
function asyncActivateFreecam ()
	if not isEnabled(source) then
		...
		if not exports.un_integration:isPlayerTrialAdmin(source) then
			exports.un_global:sendMessageToAdmins("[FREECAM] "..exports.un_global:getAdminTitle1(source).." has activated temporary /freecam.")
		end
		setElementData(source, "freecam:state", true, false)
		exports.un_logs:dbLog(source, 4, {source}, "FREECAM")
	end
end
...
function tv(player)
	local hasTV = exports.un_global:hasItem(player, 104, 1)
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if not isTVRunning() then
				...
				exports.un_logs:dbLog(player, 23, player, "TV START")
				watching = 0
				earnings = 0
		...
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				for k, v in ipairs( getElementsByType( "player" ) ) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Bağışlar: $" .. exports.un_global:formatMoney(earnings), v, 200, 100, 200)
					end
				end
				
				exports.un_logs:dbLog(player, 23, player, "TRT STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Kazanç: $" .. exports.un_global:formatMoney(earnings), player, 200, 100, 200)
			else
...
function add( affectedPlayers )
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.un_global:giveMoney(getTeamFromName"TRT Haber", LSN_MONEY * shownto)
		--exports.un_logs:dbLog(player, 23, player, "TV $" .. ( 10 * shownto ) .. " " .. message)
	else
		--exports.un_logs:dbLog(player, 23, player, "TV OFF " .. message)
	end
end