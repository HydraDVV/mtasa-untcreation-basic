function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox(exports.un_pool:getServerSyntax("Duty", "w").."Herhangi bir işbaşı bölgesinde bulunmuyorsun.", 255, 255, 255, true)
	end
end
...
function createArmourGUI()
	...
	window = guiCreateWindow(0, 0, 520, 200, "LSPD, Commerce HQ - UNTG", false)
   				...
   				elseif source == okButton then
   					if guiRadioButtonGetSelected(_25) then
						triggerServerEvent("duty.setArmor", localPlayer, localPlayer, 50)
   						outputChatBox(exports.un_pool:getServerSyntax(false, "s").."Zırh başarıyla edinildi.", 255, 255, 255, true)
   						destroyElement(window)
   						return
   					end
...
function selectPackageGUI_open(availablePackages, allowList)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		for index, itemDetails in pairs(packageDetails[5]) do
			...
			gui[guiPrefix.."pushButton1"] = guiCreateButton(xAxis, yAxis, 71, 51, exports['un_items']:getItemName(itemDetails[2]), false, gui[guiTabName])
		...
		for skinIndex, skinID in pairs(skinTable) do
			...
			gui[guiTabName.."-skin-"..skinID] = guiCreateStaticImage (xAxis, yAxis, 75, 75, ":un_auth/img/" .. skinImg..".png", false, gui[guiTabName] )
