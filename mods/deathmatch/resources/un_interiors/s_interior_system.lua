...
function setElementDataEx(source, field, parameter, streamtoall, streamatall)
	exports.un_anticheat:changeProtectedElementDataEx( source, field, parameter, streamtoall, streamatall)
end
-- End small hack
...
function findProperty(thePlayer, dimension)
	...
	if dbid and tonumber(dbid) and tonumber(dbid) > 0 then
		dbid = tonumber(dbid) 
		local foundInterior = exports.un_pool:getElement("interior", dbid)
...
function cleanupProperty( id, donotdestroy)
	if id > 0 then
		...
		if resE then
			call( getResourceFromName( "un_elevators" ), "delElevatorsFromInterior", "MAXIME" , "PROPERTYCLEANUP",  id ) 
		end
		
		if not donotdestroy then
			local res1 = getResourceRootElement( getResourceFromName( "un_object_system" ) )
			if res1 then
				exports['un_object_system']:removeInteriorObjects( tonumber(id) )
			end
		end
		
		if safeTable[id] then
			local safe = safeTable[id]
			call( getResourceFromName( "un_items" ), "clearItems", safe )
		...
		setTimer ( function () 
			call( getResourceFromName( "un_items" ), "deleteAllItemsWithinInt", id, 0, "CLEANUPINT" ) 
		end, 3000, 1)
	...
	if dbid > 0 then
		...
		elseif interiorType ~= 3 and commandName == "unrent" then
			...
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") or ( getElementData(thePlayer, "factionleader") > 0 and interiorStatus[INTERIOR_FACTION] == getElementData(thePlayer, "faction")) then
				...
				exports.un_logs:dbLog(thePlayer, 37, { "in"..tostring(dbid) } , "SELLPROPERTY "..dbid)
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney, CLEANUP)
	...
	if query then
		...
		if getElementDimension(thePlayer) == dbid and not CLEANUP then
			...
			exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "interiormarker", false, false, false)
		end

		if safeTable[dbid] then
			local safe = safeTable[dbid]
			if safe then
				exports['un_items']:clearItems(safe)
				destroyElement(safe)
				safeTable[dbid] = nil
			end
		end
		...
		if interiorType == 0 or interiorType == 1 then
			...
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				if givemoney then
					exports.un_global:giveMoney(thePlayer, money)
					if exports.un_global:takeMoney(gov, money, true) then
					end
				end
				
				if showmessages then
					if CLEANUP == "FORCESELL" then
						exports.un_global:sendMessageToAdmins("[INTERIOR]: "..exports.un_global:getPlayerFullIdentity(thePlayer).." has force-sold interior #"..dbid.." ("..getElementData(interiorElement,"name")..").") 
					else
						outputChatBox("Mülkünüzü sattınız " .. exports.un_global:formatMoney(money) .. exports.un_pool:getServerMoneyType(thePlayer).."", thePlayer, 0, 255, 0)
					end
				end
				
				-- take all keys
				call( getResourceFromName( "un_items" ), "deleteAll", interiorType == 0 and 4 or 5, dbid )
			...
			elseif interiorStatus[INTERIOR_FACTION] == getElementData(thePlayer, "faction") then
				local money = math.ceil(interiorStatus[INTERIOR_COST] * 2/3)		
				local faction = exports.un_factions:getTeamFromFactionID(interiorStatus[INTERIOR_FACTION])
				if givemoney and faction then
					exports.un_global:giveMoney(faction, money)
					if exports.un_global:takeMoney(gov, money, true) then
						--exports.un_bank:addBankTransactionLog(-getElementData(gov, "id"), -interiorStatus[INTERIOR_FACTION], money, 3 , "Interior sold to Government", getElementData(interiorElement, "name").." (ID: "..getElementData(interiorElement, "dbid")..")" )
					end
				end
				...
				if showmessages then
					if CLEANUP == "FORCESELL" then
						exports.un_global:sendMessageToAdmins("[INTERIOR]: "..exports.un_global:getPlayerFullIdentity(thePlayer).." has force-sold interior #"..dbid.." ("..getElementData(interiorElement,"name")..").") 
					elseif faction then
						outputChatBox("Mülkünüzü sattınız " .. exports.un_global:formatMoney(money) .. exports.un_pool:getServerMoneyType(thePlayer).." (Bankaya havale yapıldı: '"..getTeamName(faction).."')", thePlayer, 0, 255, 0)
					end
				end
...
function unownProperty(intid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the publicSellProperty however, it's simplier. / MAXIME / 2015.1.11
	...
	call( getResourceFromName( "un_items" ), "deleteAll", 4 , intid )
	call( getResourceFromName( "un_items" ), "deleteAll", 5 , intid )
	...
	exports.un_logs:dbLog("SYSTEM", 37, { "in"..intid } , reason and reason or "FORCESELL")
	exports["un_interior_manager"]:addInteriorLogs(intid, reason and reason or "Forcesold by SYSTEM")
		...
		elseif not targetPlayerName then
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 10 and getElementDimension(targetPlayer) == getElementDimension(thePlayer) then
					if not exports.un_global:canPlayerBuyInterior(targetPlayer) then
					...
					if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
						if getElementData(targetPlayer, "dbid") ~= interiorStatus[INTERIOR_OWNER] then
							if exports.un_global:hasSpaceForItem(targetPlayer, 4, dbid) then
								...
								if query then							
									...
									call( getResourceFromName( "un_items" ), "deleteAll", 4, dbid )
									call( getResourceFromName( "un_items" ), "deleteAll", 5, dbid )
									exports.un_global:giveItem(targetPlayer, keytype, dbid)
									...
									exports.un_logs:dbLog(thePlayer, 37, { targetPlayer, "in"..tostring(dbid) } , "SELLPROPERTY "..getPlayerName(thePlayer).." => "..targetPlayerName)
									local adminID = getElementData(thePlayer, "account:id")
									
									realReloadInterior(dbid, {targetPlayer, thePlayer})
									exports["un_interior_manager"]:addInteriorLogs(dbid, commandName.." to "..targetPlayerName.."("..getElementData(targetPlayer, "account:username")..")", thePlayer)
								else
...
function realReloadInterior(interiorID, updatePlayers)
	local dbid, entrance, exit, inttype, interiorElement = exports['un_interiors']:findProperty( false, interiorID )
	if dbid > 0 then
		if safeTable[dbid] then
	...
	dbQuery(
		function(qh)
			...
			if rows > 0 then
				...
				whatToReturn = true
				exports.un_pool:allocateElement(interiorElement, tonumber(row.id), true)
				if massLoad then
...
function buyInterior(player, pickup, cost, isHouse, isRentable, isFurniture)
	if not exports.un_global:canPlayerBuyInterior(player) then
	...
	elseif not exports.un_global:hasSpaceForItem(player, 4, 1) then
	...
	if exports.un_global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Tebrikler! Az önce bu evi satın aldınız "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.un_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.un_global:giveMoney( getTeamFromName("İstanbul Büyük Şehir Belediyesi"), cost )
		elseif (isRentable) then
			outputChatBox("Tebrikler! Az önce bu evi kiraladınız "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.un_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Tebrikler! Az önce bu işyerini aldınız "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.un_global:formatMoney(cost) .. ".", player, 255, 194, 14)
			exports.un_global:giveMoney( getTeamFromName("İstanbul Büyük Şehir Belediyesi"), cost )
		end
		...
		if (isHouse) or (isRentable) then
			exports.un_global:giveItem(player, 4, pickupid)
		else
			exports.un_global:giveItem(player, 5, pickupid)
		end
		exports.un_logs:dbLog(thePlayer, 37, { "in"..tostring(pickupid) } , "BUYPROPERTY $"..cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["un_interior_manager"]:addInteriorLogs(pickupid, "Alındı ​​/ Kiralandı, "..exports.un_pool:getServerMoneyType(thePlayer) ..exports.un_global:formatMoney(cost)..", "..getPlayerName(thePlayer), thePlayer)
	else
...
function buypropertyForFaction(interior, cost, isHouse) --Maxime
	local can, reason = exports.un_global:canPlayerFactionBuyInterior(source)
	if not can then
		outputChatBox(reason, source, 255, 0, 0)
		return
	end
	local theFaction = can
	if not exports.un_global:takeMoney(theFaction, cost) then
	...
	exports.un_global:giveMoney( gov, cost )
	--exports.un_bank:addBankTransactionLog(-factionId, -(getElementData(gov, "id")), cost, 3 , "Mülk Satın Alma", intName.." (ID: "..intId..")" )
	...
	if not dbExec(mysql:getConnection(), "UPDATE interiors SET owner='-1', faction='"..factionId.."', locked=0, lastused=NOW() WHERE id='" .. intId .. "'") then
		exports.un_global:giveMoney(theFaction, cost)
		exports.un_global:takeMoney( gov, cost )
		outputChatBox("Internal error code 334INT2.", source, 255, 0, 0)
		return false
	end
	local factionName = getTeamName(theFaction):gsub("_", " ")
	outputChatBox("Tebrikler! Bu mülkü az önce satın aldınız '"..factionName.."' "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.global:formatMoney(cost) .. ".", source, 255, 194, 14)
	...
	exports.un_global:giveItem(source, isHouse and 4 or 5, intId)
	
	exports.un_logs:dbLog(source, 37, { "in"..tostring(intId) } , "BUYPROPERTY $"..cost.." FOR FACTION '"..factionName.."'")
	realReloadInterior(tonumber(intId))
	exports["un_interior_manager"]:addInteriorLogs(intId, "'"..factionName.."', "..exports.un_pool:getServerMoneyType(thePlayer) ..exports.un_global:formatMoney(cost)..", "..getPlayerName(source), source)
	...
	exports.un_achievement:playSoundFx(source)
	return true
end
...
function buyInteriorCash(player, pickup, cost, isHouse, isRentable, isFurniture)
	...
	if exports.un_global:takeMoney(player, cost) then
		...
		elseif (isRentable) then
			outputChatBox("Tebrikler! Az önce bu mülkü kiraladınız "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.un_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		else
			outputChatBox("Tebrikler! Az önce bu işyerini satın aldınız "..exports.un_pool:getServerMoneyType(thePlayer) .. exports.un_global:formatMoney(cost) .. ".", player, 255, 194, 14)
		end
		...
		call( getResourceFromName( "un_items" ), "deleteAll", isHouse and 4 or 5, pickupid )
		exports.un_global:giveItem(player, isHouse and 4 or 5, pickupid)
		
		exports.un_logs:dbLog(player, 37, { "in"..tostring(pickupid) } , "BUYPROPERTY $"..cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["un_interior_manager"]:addInteriorLogs(pickupid, "Alındı ​​/ Kiralandı, "..exports.un_pool:getServerMoneyType(thePlayer) ..exports.un_global:formatMoney(cost)..", "..getPlayerName(player), player)
...
function buyInteriorToken(player, pickup, cost, isHouse, isRentable, isFurniture)
	...
	if exports.un_global:takeItem(player, 262, 1) then
		local charid = getElementData(player, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		local intName = getElementData(pickup, "name")
...
function buyInteriorBank(player, pickup, cost, isHouse, isRentable)
	...
	if not exports.un_bank:takeBankMoney(player, cost) then
		...
		--exports.un_bank:addBankTransactionLog(charid, -(getElementData(gov, "id")), cost, 2 , "Mülk Satın Alma", intName.." (ID: "..pickupid..")" )
		...
		exports.un_logs:dbLog(player, 37, { "in"..tostring(pickupid) } , "BUYPROPERTY $"..cost)
		realReloadInterior(tonumber(pickupid), {player})
		exports["un_interior_manager"]:addInteriorLogs(pickupid, "Bought/rented, $"..exports.un_global:formatMoney(cost)..", "..getPlayerName(player), player)
		triggerClientEvent(player, "createBlipAtXY", player, interiorEntrance[INTERIOR_TYPE], interiorEntrance[INTERIOR_X], interiorEntrance[INTERIOR_Y])
	end
end
...
function enterInterior(  )
	...
	if source and client then
		...
		elseif isInteriorForSale(source) then
			...
			local neighborhood = exports.un_global:getElementZoneName(source)
		...
		if interiorStatus[INTERIOR_OWNER] == getElementData(plr, "dbid") then
			...
			if interiorStatus[1] == 1 then
				...
				if durum == "cek" then
					...
					realReloadInterior(intdbid)
					exports.un_global:giveMoney(plr, miktar)
...
local interiorStatus = getElementData(theInterior, "status")
	...
	if interiorStatus[1] == 1 then
		...
		if getElementDimension(thePlayer) == 0 then
			...
			if not exports.un_global:takeMoney(thePlayer, girispara) then
		...
		setElementData(theInterior, "lastused", exports.un_datetime:now(), true)

		exports.un_logs:dbLog("SYSTEM", 31, { theInterior, thePlayer } , enter and "ENTERED" or "EXITED")
		exports["un_interior_manager"]:addInteriorLogs(dbid, enter and "Entered" or "Exited", thePlayer)
...
function addSafeAtPosition( thePlayer, x, y, z, rotz )
	...
	elseif dbid >= 20000 then -- Vehicle Interiors
		...
		if exports['un_vehicle']:getSafe( vid ) then
			outputChatBox("Bu mülkte zaten bir kasa var. Taşımak için /movesafe yazın.", thePlayer, 255, 0, 0)
			return 1
		elseif exports.un_global:hasItem( thePlayer, 3, vid ) then
			z = z - 0.5
			rotz = rotz + 180
			if dbExec(mysql:getConnection(), "UPDATE vehicles SET safepositionX='" .. x .. "', safepositionY='" .. y .. "', safepositionZ='" .. z .. "', safepositionRZ='" .. rotz .. "' WHERE id='" .. vid .. "'") then
				if exports['un_vehicle']:addSafe( vid, x, y, z, rotz, interior ) then
					return 0
				end
			end
			return 1
	...
	elseif dbid >= 19000 then -- temp vehicle interiors
		return 2
	elseif ((exports.un_global:hasItem( thePlayer, 5, dbid ) or exports.un_global:hasItem( thePlayer, 4, dbid))) then
		if safeTable[dbid] then
			outputChatBox("Bu mülkte zaten bir kasa var. Taşımak için /movesafe yazın.", thePlayer, 255, 0, 0)
			return 1
		else
...
function moveSafe ( thePlayer, commandName )
	...
	local dbid = getElementDimension( thePlayer )
	local interior = getElementInterior( thePlayer )
	if (dbid < 19000 and (exports.un_global:hasItem( thePlayer, 5, dbid ) or exports.un_global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.un_global:hasItem(thePlayer, 3, dbid - 20000)) then
		z = z - 0.5
		rotz = rotz + 180
		if dbid >= 20000 and exports['un_vehicle']:getSafe(dbid-20000) then
			local safe = exports['un_vehicle']:getSafe(dbid-20000)
...
local function hasKey( source, key )
	if exports.un_global:hasItem(source, 4, key) or exports.un_global:hasItem(source, 5,key) then
		return true, false
	else
...
function lockUnlockHouseEvent(player, checkdistance)
	...
	local possibleInteriors = exports.un_pool:getPoolElementsByType("interior")
	...
	local possibleElevators = exports.un_pool:getPoolElementsByType("elevator")
	...
	if found and itemValue then
		...
		if getElementData(interiorElement, "keypad_lock") then
			if not (exports.un_integration:isPlayerTrialAdmin(source) and getElementData(source, "duty_admin") == 1) then
				exports.un_hud:sendBottomNotification(source, "Keyless Digital Door Lock", "This door is keyless, you must use the keypad to access it.")
				return false
			end
		end
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.un_global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.un_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") has unlocked Interior ID #"..itemValue.." without key.")
					exports.un_global:sendLocalText(source, " * Kapı şuan kilidi açık *", 255, 51, 102, 30, {}, true)
					exports["un_interior_manager"]:addInteriorLogs(itemValue, "unlock without key", source)
				end
			else
				exports.un_global:sendLocalMeAction(source, " kilidi açmak için anahtarı kapıya koyar.")
			end
			exports.un_logs:dbLog(source, 31, {  "in"..tostring(itemValue) }, "UNLOCK INTERIOR")
		else --shit
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.un_global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.un_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") has locked Interior ID #"..itemValue.." without key.")
					exports.un_global:sendLocalText(source, " * Kapı şuan kilitli *", 255, 51, 102, 30, {}, true)
					exports["un_interior_manager"]:addInteriorLogs(itemValue, "lock without key", source)
				end
			else 
				exports.un_global:sendLocalMeAction(source, " kilitlemek için anahtarı kapıya yerleştirir.")
			end
			exports.un_logs:dbLog(source, 31, {  "in"..tostring(itemValue) }, "LOCK INTERIOR")
		end
		
		interiorStatus[INTERIOR_LOCKED] = newRealLockedValue
		exports.un_anticheat:changeProtectedElementDataEx(interiorElement, "status", interiorStatus, true)	
	else
		cancelEvent( )
	end
end
...
function doorUnlockSound(house, source)
	if (house) then
		if (getElementType(house) == "interior") then
			...
			for index, nearbyPlayer in ipairs(exports.un_pool:getPoolElementsByType("player")) do
	...
	function( id, usingKeypad )
		...
		if id and tonumber(id) and (hasKey1 or usingKeypad) then
			...
			if not usingKeypad then
				...
				if locked == 0 then
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.un_global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.un_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") has unlocked Interior ID #"..id.." without key.")
							exports.un_global:sendLocalText(source, " * Kapı kilidi şuan açık *", 255, 51, 102, 30, {}, true)
							exports["un_interior_manager"]:addInteriorLogs(id, "unlock without key", source)
						end
					else
						exports.un_global:sendLocalMeAction(source, " kilidi açmak için anahtarı kapıya koyar")
					end
					exports.un_logs:dbLog(source, 31, {  "in"..tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.un_global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.un_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") has locked Interior ID #"..id.." without key.")
							exports.un_global:sendLocalText(source, " * Kapı şuan kilitli *", 255, 51, 102, 30, {}, true)
							exports["un_interior_manager"]:addInteriorLogs(id, "lock without key", source)
						end
					else
						exports.un_global:sendLocalMeAction(source, " kilitlemek için anahtarı kapıya yerleştirir.")
					end
					exports.un_logs:dbLog(source, 31, {  "in"..tostring(id) }, "LOCK INTERIOR")
				end
				...
				if locked == 0 then
					exports.un_global:sendLocalMeAction(source, "kapının kilidini açar.")
					exports.un_logs:dbLog(source, 31, {  "in"..tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					exports.un_global:sendLocalMeAction(source, "kapıyı kilitler.")
					exports.un_logs:dbLog(source, 31, {  "in"..tostring(id) }, "LOCK INTERIOR")
				end
...
function client_requestHUDinfo()
	...
	exports.un_anticheat:changeProtectedElementDataEx( client, "interiormarker", true, false, false )
	...
	local interiorOwnerName = exports['un_cache']:getCharacterName(interiorStatus[INTERIOR_OWNER]) or "None"
...
addEventHandler("int:updatemarker", getRootElement(), 
	function( newState )
		exports.un_anticheat:changeProtectedElementDataEx(client, "interiormarker", newState, false, true) -- No sync at all: function is only called from client thusfar has the actual state itself
	end
)
