...
function interiorShowPickups(interiorElement, isElevator)
    ...
    if not isElevator then
        ...
        if intStatus[INTERIOR_OWNER] > 0 then
            local ownerName = exports.un_cache:getCharacterNameFromID(intStatus[INTERIOR_OWNER])
            if intType == 3 then
        ...
        elseif intStatus[INTERIOR_FACTION] > 0 then
            local ownerName = exports.un_cache:getFactionNameFromId(intStatus[INTERIOR_FACTION])
            if intType ~= 2 then
            ...
            elseif intType == 3 then
                local intPrice = exports.un_global:formatMoney(intStatus[INTERIOR_COST])
                intOwner = intPrice
            else
                local intPrice = exports.un_global:formatMoney(intStatus[INTERIOR_COST])
...
function canPlayerKnowInteriorOwner(theInterior)
    return  (getElementData(theInterior, "status")[INTERIOR_OWNER] == 0) -- unown.
        or  (exports.un_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
...
function canPlayerSeeInteriorID(theInterior)
    return  getElementData(localPlayer, "faction") == 1 -- LSPD
        ...
        or  (exports.un_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
        or  (exports.un_integration:isPlayerSupporter(localPlayer) and (getElementData(localPlayer, "duty_supporter") == 1))
end
...
function forcePickupSpawn()
    if exports.un_integration:isPlayerScripter(getLocalPlayer()) then
        initializeSoFar()
    end
end
