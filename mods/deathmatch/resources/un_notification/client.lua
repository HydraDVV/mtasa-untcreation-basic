...
local notiSettings = {
  ...
  font = exports.un_fonts:getFont("Roboto", 10),
...
function Notification:draw()
  ...
  if self.title then
    dxDrawText(self.title, self.x + 10/zoom + self.textImgMove, self.y + 5/zoom, self.x + notiSettings.field.w - 10/zoom, self.y + self.h/2, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, notiSettings.font, "left", "bottom", true, true, true, self.useColor)
    dxDrawText(self.text, self.x + 10/zoom + self.textImgMove, self.y + self.h/2, self.x + notiSettings.field.w - 10/zoom, self.y + self.h - notiSettings.field.bar - 5/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, notiSettings.font, "left", "top", true, true, true, self.useColor)
  else
    dxDrawText(self.text, self.x + 10/zoom + self.textImgMove, self.y + 5/zoom, self.x + notiSettings.field.w - 10/zoom, self.y + self.h - notiSettings.field.bar - 5/zoom, tocolor(200, 200, 200, 255 * self.alpha), 1/zoom, notiSettings.font, "left", "center", true, true, true, self.useColor)
  end

  if self.count > 1 then
    dxDrawText("x"..self.count, self.x, self.y, self.x + notiSettings.field.w - 5/zoom, self.y + self.h - notiSettings.field.bar - 2/zoom, tocolor(160, 160, 160, 255 * self.alpha), 1/zoom, notiSettings.font, "right", "bottom", false, false, true)
  end
...
function Notification:getTextRows()
  ...
  for i, v in pairs(sentences) do
    ...
    if #words > 0 then
      while (#words > 0) do
        local textLong = dxGetTextWidth(text, 1/zoom, notiSettings.font)
