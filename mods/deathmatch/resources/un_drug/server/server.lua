...
function create(thePlayer, cmd)
	if thePlayer:getData("loggedin") == 1 then
		if exports["un_items"]:hasItem(thePlayer, seedItemID) and exports["un_items"]:hasItem(thePlayer, potItemID) then
			...
			if query then
				exports.un_global:sendLocalMeAction(thePlayer, "yavaşça eğilerek sağ/sol elindeki saksıyı yere koyar.", false, true)
				exports.un_global:sendLocalMeAction(thePlayer, "sağ/sol elindeki tohumu saksıya eker.", false, true)
				...
				if owner == pdbid then
					if pots[ID].object:getData("state") >= 100 then
						...
						if query then
							...
							pots[ID].plant:destroy()
							pots[ID] = nil
							exports.un_global:sendLocalMeAction(thePlayer, "yavaşça eğilerek yerdeki saksıdaki marijuanayı toplar ve saksıyı kaldırır.", false, true)
