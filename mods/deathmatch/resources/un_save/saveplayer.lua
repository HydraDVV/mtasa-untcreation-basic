...
function syncTIS()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (tis) and (getPlayerIdleTime(value) < 600000)  then
			exports.un_anticheat:changeProtectedElementDataEx(value, "timeinserver", tonumber(tis)+1, false)
		end
	end
end
...
addCommandHandler("saveall", function(p) if exports.un_integration:isPlayerSeniorAdmin(p) then beginSave() outputChatBox("Done.", p) end end)
