...
function playerDeath(totalAmmo, killer, killerWeapon)
	if getElementData(source, "dbid") then
		...
		if getElementData(source, "adminjailed") then
			...
			exports.un_logs:dbLog(source, 34, source, "died in admin jail")
		elseif getElementData(source, "pd.jailtimer") then
			...
			exports.un_logs:dbLog(source, 34, source, "Hücrede öldü")
		else
			local affected = { }
			...
			if 	(getElementData(source, "seatbelt") == true) then
				exports.un_anticheat:changeProtectedElementDataEx(source, "seatbelt", false, true)
			end
			
			if 	(getElementData(source, "fishing") == true) then
				exports.un_anticheat:changeProtectedElementDataEx(source, "fishing", false, true)
				triggerClientEvent("fishing:killTimers", source)
			end
			
			if 	(getElementData(source, "restrain") == true) then
				exports.un_anticheat:changeProtectedElementDataEx(source, "restrain", false, true)
			end
			...
			exports.un_logs:dbLog(source, 34, affected, killstr)
			exports.un_anticheat:changeProtectedElementDataEx(source, "lastdeath", " [KILL] "..getPlayerName(source):gsub("_", " ") .. killstr, true)
		end
	end
end
...
function acceptDeath(thePlayer, victimDropItem)
	if getElementData(thePlayer, "dead") == 1 then
		if victimDropItem then
			...
			for key, item in pairs(exports["un_items"]:getItems(thePlayer)) do 
				itemID = tonumber(item[1])
				local ammo = false
				if itemID == 116 then 
					ammo = exports.un_global:explode( ":", item[2]  )[2]
				end
...
function logMe( message )
	...
	local r = getRealTime()
	exports.un_global:sendMessageToAdmins(message)
...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		if (getElementData(thePlayer, "loggedin") == 0) then
			exports.un_global:sendMessageToAdmins("AC0x0000004: "..getPlayerName(thePlayer):gsub("_", " ").." died while not in character, triggering blackfade.")
			return
		end
		...
		exports.un_global:giveMoney( getTeamFromName("Los Santos Acil Servisleri"), math.ceil((1-tax)*cost) )
		exports.un_global:takeMoney( getTeamFromName("Los Santos Hükümeti"), math.ceil((1-tax)*cost) )
		...
		if removedWeapons ~= nil then
			logMe(death)
			exports.un_global:sendMessageToAdmins("/showkills to view lost weapons.")
		...
		if getElementData(source, "hunger") <= 0 then
			exports.un_anticheat:changeProtectedElementDataEx(source, "hunger", 5)
		end
		if getElementData(source, "thirst") <= 0 then
			exports.un_anticheat:changeProtectedElementDataEx(source, "thirst", 5)
		end
	...
	if not (targetPlayer) or not (duration) then
		...
		if targetPlayer then
			...
			if (logged==1) then
				...
				if (factionType==4) or (exports.un_integration:isPlayerTrialAdmin(thePlayer) == true) then
						local dimension = getElementDimension(thePlayer)
							totaltime = tonumber(duration)
							if totaltime < 12 then
								local money = exports.un_bank:takeBankMoney(targetPlayer, 100*totaltime)
								if not money then
									outputChatBox("This player does not have enough money in their bank to be placed in recovery.", thePlayer, 255, 0, 0)
									return 
								end
								exports.un_global:giveMoney( getTeamFromName("Los Santos Fire Department"), 100*totaltime )
								...
								exports.un_global:sendMessageToAdmins("AdmWrn: " .. targetPlayerName .. " was put in recovery for " .. duration .. " hour(s) by " .. getPlayerName(thePlayer):gsub("_"," ") .. ".")
								...
								exports.un_logs:dbLog(thePlayer, 4, targetPlayer, "RECOVERY " .. duration)
								local r = getRealTime()
	...
	if (getTeamName(team)=="Los Santos Fire Department") then
		if not (...) then
			...
			if not(itemValue=="") then
				exports.un_global:giveItem( thePlayer, 132, itemValue )
				outputChatBox("The prescription '" .. itemValue .. "' has been processed.", thePlayer, 0, 255, 0)
				exports.un_global:sendMessageToAdmins(getPlayerName(thePlayer):gsub("_"," ") .. " has made a prescription with the value of: " .. itemValue .. ".")
				exports.un_logs:dbLog(thePlayer, 4, thePlayer, "PRESCRIPTION " .. itemValue)
			end
		end
	end
end
			...
			if targetPlayer then
				if getElementData(targetPlayer, "dead") == 1 then
					...
					if getElementData(targetPlayer, "hunger") <= 0 then
						exports.un_anticheat:changeProtectedElementDataEx(targetPlayer, "hunger", 5)
					end
					if getElementData(targetPlayer, "thirst") <= 0 then
						exports.un_anticheat:changeProtectedElementDataEx(targetPlayer, "thirst", 5)
					end
					...
					local adminTitle = tostring(exports.un_global:getPlayerAdminTitle(thePlayer))
					exports.un_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncuyu canlandırdı.")
					outputChatBox("You have been revived by "..tostring(exports.un_global:getPlayerAdminTitle(thePlayer)).." "..tostring(getPlayerName(thePlayer):gsub("_"," "))..".", targetPlayer, 0, 255, 0)
