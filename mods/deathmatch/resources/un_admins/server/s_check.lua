...
function doCheck(sourcePlayer, command, ...)
	if (exports.un_integration:isPlayerTrialAdmin(sourcePlayer) or exports.un_integration:isPlayerSupporter(sourcePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. command .. " [Partial Player Name / ID]", sourcePlayer, 255, 194, 14)
		else
			local noob = exports.un_global:findPlayerByPartialNick(sourcePlayer, table.concat({...},"_"))
			if (noob) then
									...
									function(qh)
										...
										dbQuery(
											function(qh)
												...
												local adminlevel = exports.un_global:getPlayerAdminTitle(noob)
				...
				exports.un_logs:dbLog(thePlayer, 4, ..., "CHECK")
			end
		end
	end
end
...
function savePlayerNote( target, text )
	if exports.un_integration:isPlayerTrialAdmin(client) or exports.un_integration:isPlayerSupporter(client) then
...
function showAdminHistory( target )
	...
	if not (exports.un_integration:isPlayerTrialAdmin( client ) or exports.un_integration:isPlayerSupporter( client )) then
				...
				if (tonumber(sqlQuery["action"]) == 4) then -- Warning
					...
					for i, player in pairs(getElementsByType("player")) do
						if getElementData(player, "account:id") == tonumber(accountNumber) then
							...
							exports.un_anticheat:changeProtectedElementDataEx(player, "warns", warns, false)
							break
						end
					end
				end
...
addCommandHandler( "history", 
	function( thePlayer, commandName, ... )
		if not (exports.un_integration:isPlayerTrialAdmin(thePlayer) or exports.un_integration:isPlayerSupporter(thePlayer)) then
			if (...) then
		...
		if (...) then
			targetPlayer = exports.un_global:findPlayerByPartialNick(thePlayer, table.concat({...},"_"))
		end
