		...
		if (itemCheck[1] == 115) then -- Weapon
			...
			if firstClipBullets == 0 then
				...
				if not (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], true, false, false)
				end
			else
				if (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], false, false, false)
				end
			end
...
function reloadA(thePlayer,weapon,ammocalc)
	...
	exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "reloading", true, false, false)
	setTimer(checkFalling, 100, 10, thePlayer)
	exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..weapon, false, false, false)
	if not (isPedDucked(thePlayer)) and not isPedInVehicle(thePlayer) then
		exports.un_global:applyAnimation(thePlayer, "BUDDY", "buddy_reload", 1000, false, true, true)
		...
		if not foundGun then
			for _, clientWeaponRow in ipairs(ammoTable) do
				if tonumber(clientWeaponRow[1]) == tonumber(weaponID) then
					...
					--exports.un_global:sendMessageToAdmins("[AntiCheat] 0x00000010 detected on "..getPlayerName(client) .. " (has clientside weapon ("..tostring(weaponID)..") that dont exist serverside)")
				end
			end
		end
...
function checkFalling(thePlayer)
	...
	if not (isPedOnGround(thePlayer)) and (reloading) then
		-- reset state
		exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "reloading.timer", false, false)
		exports.un_global:removeAnimation(thePlayer)
		exports.un_anticheat:changeProtectedElementDataEx(thePlayer, "reloading", false, false)
...
function giveReload(thePlayer, weapon, ammo)
	...
	--exports.un_global:setWeaponAmmo(thePlayer, weapon, ammo, clipsize) -- fix for the ammo adding up bug
