...
local function getHoverElement( resource, force )
	...
	for _, acceptProtected in ipairs({false, true}) do
		...
		if element and getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName(resource or "un_item_world")) then
			return element
		elseif b and c and d and not resource then
			element = nil
			local x, y, z = nil
			local maxdist = 0.34
			for key, value in ipairs(getElementsByType("object",getResourceRootElement(getResourceFromName(resource or "un_item_world")))) do
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
			...
			if show then
				...
				for i = irows, jrows do
					...
					if i == activeTab then
						exports.un_draw:shadow(boxx,boxy,box,box,0,0,0,200,5,true)
					else
						exports.un_draw:shadow(boxx,boxy,box,box,0,0,0,200,1,true)
					end
					dxDrawRectangle(boxx,boxy+box-1,box,1,tocolor(226, 192, 74))
					...
					if not isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementType( hoverElement ) == "ped" and  getElementData(hoverElement,"customshop") then --  import products to custom shop
								color = background_movetoelement_color
							elseif getElementParent(getElementParent(hoverElement)) ~= getResourceRootElement(getResourceFromName("un_item_world")) or hoverElement == getHoverElement() then
								color = full_color
							elseif tonumber(getElementData(hoverElement, "itemID")) == 166 and item[1] == 165 then -- Video disc into video player /Exciter
								color = background_movetoelement_color
			...
			elseif not tooltipYet and getPedOccupiedVehicle(localPlayer) then
				...
				element = getHoverElement("vehicle") or getHoverElement("un_admins")
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['un_vehicle']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
...
addEventHandler( "onClientClick", getRootElement( ),
	function( button, state, cursorX, cursorY, worldX, worldY, worldZ )
		if not waitingForItemDrop then
			if button == "left" or ( button == "middle" and exports.un_integration:isPlayerTrialAdmin( getLocalPlayer( ) ) ) then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SHOW then
								...
								elseif item[1] == 150 then--ATM card
									...
									local owner = exports.un_cache:getCharacterNameFromID(itemExploded[2])
							...
							elseif hoverAction == ACTION_SPLIT then
								...
								elseif itemID == 115 then
									if exports.un_integration:isPlayerTrialAdmin(localPlayer) then -- restrict this to trial admin+ / maxime
										renameWeapon(item)
									else
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.un_integration:isPlayerAdmin(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.un_integration:isPlayerAdmin(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
...
function playSoundInvOpen()
	setSoundVolume(playSound(":un_resources/inv_open.mp3"), 0.3)
end

function playSoundInvClose()
	setSoundVolume(playSound(":un_resources/inv_close.mp3"), 0.3)
end