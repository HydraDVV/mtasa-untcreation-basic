...
local function forceUpdate( )
	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			if interiorType==2 then
				if (not exports.un_factions:isInFactionType(getLocalPlayer(), 2) and not exports.un_factions:isInFactionType(getLocalPlayer(), 3) and getElementData(getLocalPlayer(), "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
	...
	if getElementType( element ) == "vehicle" then
		...
		if exports.un_integration:isPlayerTrialAdmin(getLocalPlayer()) then
...
function hasKey( key, element )
	...
	elseif getElementData(getLocalPlayer(), "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.un_factions:isInFactionType(getLocalPlayer(), 2) and not exports.un_factions:isInFactionType(getLocalPlayer(), 3)) then
				return false, false
			else
				return true, true
			end
...
local function moveFromElement( button )
	...
	if button == "left" and col ~= -1 and row ~= -1 then
		...
		if slot < 0 --[[and exports.un_integration:isPlayerTrialAdmin(localPlayer)]] then
			...
			if item then
				...
				if (hasKey(getElementData(element, "dbid"), element) and getElementType(element)=="interior") or getElementType(element)~="interior" then
					if itemID < 0 and itemID ~= -100 then -- weapon
						local free, totalfree = exports.un_weaponcap:getFreeAmmo( -itemID )
						local cap = exports.un_weaponcap:getAmmoCap( -itemID )
						if totalfree == 0 then
...
local function adminCheck( button )
	...
	if button == "right" and col ~= -1 and row ~= -1 then
		...
		if exports.un_global:isStaffOnDuty(getLocalPlayer()) then
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		if not elementtype then
			if getElementParent(getElementParent(source)) == getResourceRootElement(getResourceFromName("un_item_world")) then
		...
		wInventory = guiCreateStaticImage(ax-w/2, ay-h/2, w, h, ":un_resources/window_body.png",false)
...
function collisionFix()
	for key, value in pairs( getElementsByType( "object", getResourceRootElement( getResourceFromName("un_item_world") ) ) ) do
