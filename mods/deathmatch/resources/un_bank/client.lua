BankClass = {
    peds = {
        {2246.052734375, -1828.3486328125, 1625.6234130859, 9, 101, 177.48956298828}
    ...
    onReady = function(self)
        for index, value in ipairs(BankClass.peds) do
			...
			setPedAnimation(banking_ped, "COP_AMBIENT", "Coplook_think")
			...
			BankClass.timer = Timer(
				function()
					if isElement(BankClass.BankBrowser) and BankReceiver then
						BankReceiver:executeJavascript("change('curbalance', '"..exports.un_global:formatMoney(localPlayer:getData("bankmoney")).."')");
...
addEventHandler('BankBrowser-DepositMoney', root, function(amount) if exports.un_global:hasMoney(localPlayer, tonumber(amount)) then triggerServerEvent('BankBrowser-DepositMoney', localPlayer, tonumber(amount)) BankReceiver:executeJavascript("change('curbalance', '"..(localPlayer:getData("bankmoney")+tonumber(amount)).."')"); else outputChatBox("Bank of Santos:#ffffff Yeterli paranız olmadığı için parayı kabul edemiyoruz.", 100, 100, 100, true) end end)
