...
function showsize(thePlayer)
	if not exports.un_integration:isPlayerScripter(thePlayer) then
		return false
	end
...
addEventHandler("onResourceStop", resourceRoot, function () --Maxime
	exports.un_data:save(poolTable, "poolTable")
	exports.un_data:save(indexedPools, "indexedPools")
end)

addEventHandler("onResourceStart", resourceRoot, function() --Maxime
	local loaded = exports.un_data:load("poolTable")
	if loaded then
		poolTable = loaded
	end
	loaded = exports.un_data:load("indexedPools")
	if loaded then
		indexedPools = loaded
	end
