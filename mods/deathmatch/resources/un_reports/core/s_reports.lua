...
function toggleStatus(thePlayer, section, status)
	...
	exports.un_global:sendWrnToStaff(getPlayerName(thePlayer).." has toggled "..section.." to "..newstatus, "STATUS")
end
...
function resourceStart(res)
	reports = exports.un_data:loadReports() or {}
end
addEventHandler("onResourceStart", getResourceRootElement(), resourceStart)

function resourceStop(res)
	exports.un_data:saveReports(reports)
end
...
function getAdminCount()
	...
	for key, value in ipairs(getElementsByType("player")) do
		if (isElement(value)) then
			...
			if exports.un_integration:isPlayerSupporter(value) then
				gm = gm + 1
...
function updateReportCount()
	...
	for k, v in ipairs(getElementsByType("player")) do
		...
		open[v] = 0
		handled[v] = 0
		if exports.un_integration:isPlayerStaff(v) and getElementData(v, "loggedin") == 1 then
			local alreadyTold = {}
			for key, value in pairs(reports) do
				local staff, _, n, abrv = getReportInfo(value[7])
				if staff then
					for g, u in ipairs(staff) do
						if (exports.un_integration:isPlayerSupporter(v) or value[5] == v) and not alreadyTold[key] then
							open[v] = open[v] + 1
	...
	for key, value in ipairs(getElementsByType("player")) do
		if exports.un_integration:isPlayerStaff(value) then
			if exports.un_integration:isPlayerTrialAdmin(value) then
				str = ":: "..gmduty.."/"..gm.." SUP :: " .. duty .."/".. online .." admins"
			elseif exports.un_integration:isPlayerSupporter(value) then
				str = ":: "..gmduty.."/"..gm.." SUP"
			else
				str = ""
			end
...
function maximeReportsReminder()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (exports.un_integration:isPlayerSupporter(value) and god == 1) or (exports.un_integration:isPlayerTrialAdmin(value) and aod == 1) then
...
function sortReports(showMessage) --MAXIME
	...
	if showMessage then
		if adminNotice ~= "" then
			...
			local admins = exports.un_global:getAdmins()
		...
		if gmNotice ~= "" then
			...
			local gms = exports.un_global:getGameMasters()
...
function reportInfo(thePlayer, commandName, id)
	if exports.un_integration:isPlayerStaff(thePlayer) then
		if not (id) then
...
function playerQuit()
						...
						if string.find(auxiliaryTeams, usergroup) then
							for key, value in ipairs(getElementsByType("players")) do
								if getElementData(value, "loggedin") == 1 then
									if exports.un_integration:isPlayerSupporter(value) and not alreadyTold[value] then
	...
	if exports.un_integration:isPlayerStaff(source) then -- Check if a Aux staff member went offline and there is noone left to handle the report.
		for i = 1, #reports do
			if reports[i] then
	...
	if (requested=="requested") then
		for key, value in ipairs(exports.un_global:getAdmins()) do
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			if not skipadmin then
				local GMs = exports.un_global:getAdmins()
				-- Show to GMs
		...
		elseif string.find(auxiliaryTeams, usergroup) then -- Auxiliary Teams
			for key, value in ipairs(getElementsByType("player")) do
				if getElementData(value, "loggedin") == 1 then
					if exports.un_integration:isPlayerSupporter(value) then -- Opens up functionality to have reports ONLY go to leaders or only members
...
function subscribeToAdminsReports(thePlayer)
	if exports.un_integration:isPlayerSupporter(thePlayer) then
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if not found and not exports.un_integration:isPlayerSeniorAdmin(thePlayer) then
					...
					--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " maked a report as false. Report: " .. reason )
					sortReports(true)
					updateReportCount()
				end
			end
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if reportingPlayer == thePlayer and not exports.un_integration:isPlayerHeadAdmin(thePlayer) then
...
function acceptAdminReport(thePlayer, commandName, id, ...)
	...
	if (exports.un_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			local targetAdmin, username = exports.un_global:findPlayerByPartialNick(thePlayer, adminName)
			if targetAdmin then
				local id = tonumber(id)
				if not (reports[id]) then
					outputChatBox(syntaxTable["e"].."Rapor bulunamadı!", thePlayer, 255, 0, 0)
					...
					if (reportHandler) then
						...
						local timestring = hours .. ":" .. minutes
						local playerID = getElementData(reportingPlayer, "playerid")
						local adminTitle = exports.un_global:getPlayerAdminTitle(targetAdmin)
						...
						--local accountID = getElementData(thePlayer, "account:id")
						--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " was assigned a report. Report: " .. reason )
						sortReports(false)
						updateReportCount()
					end
				end
	...
	if (exports.un_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			if targetAdmin then
				...
				if not (reports[id]) then
					outputChatBox(syntaxTable["e"].."Rapor bulunamadı!", thePlayer, 255, 0, 0)
				elseif (reports[id][5] ~= thePlayer) and not (exports.un_integration:isPlayerAdmin(thePlayer)) then
					...
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " had a report transfered to them. Report: " .. reason )
					sortReports(false)
					updateReportCount()
				end
			end
	...
	if exports.un_integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				--local accountID = getElementData(thePlayer, "account:id")
				--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reporter, reported}, getPlayerName(thePlayer) .. " closed a report. Report: " .. reason )
		...
		if not (id) then
			...
			if (reports[id] == nil) then
				...
				if (reports[id][5] ~= thePlayer) then
					...
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " dropped a report. Report: " .. reason )
					sortReports(false)
					updateReportCount()
				end
			end
...
function endReport(thePlayer, commandName)
	...
	if not adminreport or not report then
		...
		-- end
		--exports.un_logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, affected, getPlayerName(thePlayer) .. " accepted a report. Report: " .. reason )
		sortReports(true)
		updateReportCount()
	end
end
	...
	if (exports.un_integration:isPlayerTrialAdmin(thePlayer) or exports.un_integration:isPlayerSupporter(thePlayer)) then
			...
			for i = 1, #reports do
				...
				if report then
					...
					if staff then
						for k,v in ipairs(staff) do
							if exports.un_integration:isPlayerTrialAdmin(thePlayer) and not seenReport[i] then
...
function updateStaffReportCount(thePlayer)
	...
	if adminreports_saved >= reportsToAward then
		...
		--exports.un_achievement:awardPlayer(thePlayer, false, "Handled "..reportsToAward.." reports!", gcToAward)
		--exports.un_global:sendWrnToStaff(exports.un_global:getPlayerFullIdentity(thePlayer).." has won "..gcToAward.." GC for completing "..reportsToAward.." reports.", "ACHIEVEMENT")
	else
