addCommandHandler("rapor",
	function(cmd, ...)
		if getElementData(localPlayer, "loggedin") == 1 then
			if not (...) then
				outputChatBox(exports.un_pool:getServerSyntax(false, "e").."/rapor <bilgi>", 255, 255, 255, true)
				return
			end
...
addCommandHandler("sorusor",
	function(cmd, ...)
		if getElementData(localPlayer, "loggedin") == 1 then
			if not (...) then
				outputChatBox(exports.un_pool:getServerSyntax(false, "w").."/sorusor <soru içeriği>", 255, 255, 255, true)
				return
			end
			local message = table.concat({...}, " ")
