function tazerFired(x, y, z, target)
	...
	if (distance<20) then
		if (isElement(target) and getElementType(target)=="player") then
			for key, value in ipairs(exports.un_global:getNearbyElements(target, "player", 20)) do
				if (value~=source) then
					triggerClientEvent(value, "showTazerEffect", value, x, y, z) -- show the sparks
				end
			end
			exports.un_anticheat:changeProtectedElementDataEx(target, "tazed", true, false)
			toggleAllControls(target, false, true, false)
			triggerClientEvent(target, "onClientPlayerWeaponCheck", target)
			exports.un_global:applyAnimation(target, "ped", "FLOOR_hit_f", 0, false, true, true, true, true)
...
addCommandHandler("tazerkaldir", 
	function(thePlayer, cmd, targetPlayer)
		...
		if getElementData(theTeam, "type") == 2 or exports.un_integration:isPlayerAdmin(thePlayer) then
...
function removeAnimation(thePlayer)
	if (isElement(thePlayer) and getElementType(thePlayer)=="player") then
		exports.un_global:applyAnimation(target, "ped", "FLOOR_hit_f", -1, false, true, true, true, true)
		exports.un_global:removeAnimation(thePlayer, true)
...
function updateDeagleMode(mode)
	if ( tonumber(mode) and (tonumber(mode) >= 0 and tonumber(mode) <= 2) ) then
		exports.un_anticheat:changeProtectedElementDataEx(client, "deaglemode", mode, true)
	end
end
