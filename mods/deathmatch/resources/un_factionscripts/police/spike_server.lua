		...
		local rotz = getPedRotation(sourcePlayer)
			...
			for value=1,SpikeLimit,1 do
				if(Spike[value] == nil) then
					...
					exports.un_pool:allocateElement(Spike[value])
					...
					exports.un_pool:allocateElement(Shape1[value])
					Shape2[value] = createColRectangle( (nx), (ny), 5.0, 5.0 )
					exports.un_pool:allocateElement(Shape2[value])
					exports.un_anticheat:changeProtectedElementDataEx(Shape1[value], "type", "spikes")
					exports.un_anticheat:changeProtectedElementDataEx(Shape2[value], "type", "spikes")
					outputChatBox("Spawned spikes with ID:" .. value, sourcePlayer, 0, 194, 0)
					exports.un_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Spawned spike ID "..tostring(value))
					break
				end
			end
		else
		...
		outputChatBox("SYNTAX: /removespikes [ID]", sourcePlayer, 255, 194, 14)
			...
			if(Spike[message] ~= nil) then
				...
				if (getDistanceBetweenPoints3D(x1, y1, z1, x2, y2, z2) <= 10) then
					...
					exports.un_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Spawned spike ID "..tostring(message))
					TotalSpikes = TotalSpikes -1
...
function AdminRemovingSpikes(sourcePlayer, command)
	if	(exports.un_integration:isPlayerTrialAdmin(sourcePlayer)) then
		for value=1,SpikeLimit,1 do
			if(Spike[value] ~= nil) then
		...
		exports.un_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Removed all the spawned spikestrips")
		TotalSpikes = nil
	end
end
