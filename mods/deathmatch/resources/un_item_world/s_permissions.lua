...
function getItemPropertiesData(object)
	...
	if creator > 0 then
		creatorName = exports.un_global:getCharacterNameFromID(creator)
	end
...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("un_item_world")) then
		...
		if id then
			...
			local itemName = exports['un_items']:getItemName(tonumber(getElementData(object, "itemID")), getElementData(object, "itemValue"))
			...
			exports.un_anticheat:changeProtectedElementDataEx(object, "worlditem.permissions", permissions)
