		...
		if gui.main and isElement(gui.main) then
			...
			addEventHandler("onClientGUIClick", gui.refresh, function()
				if source == gui.refresh then
					...
					exports.un_global:playSoundCreate()
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					if exports.un_integration:isPlayerLeadAdmin(localPlayer) then
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if owner == username or exports.un_integration:isPlayerLeadAdmin(localPlayer) then
...
function openNewStation(id, name, ip, state, order, buyNew)
	...
	exports.un_global:centerWindow(gui.wNewStation)
	local margin = 10
	local lineH = 25
	local col1 = 85
...
function renewStation(station)
	...
	exports.un_global:centerWindow(gui.wRenewStation)
	local margin = 20
	local lineH = 16
