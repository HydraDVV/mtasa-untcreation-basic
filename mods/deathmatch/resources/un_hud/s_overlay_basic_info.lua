function showStats(thePlayer, commandName, targetPlayerName)
	...
	for key, value in ipairs(exports.un_pool:getPoolElementsByType("vehicle")) do
		...
		if (owner) and (owner==dbid) then
			...
			carids = carids .. id .. ", "
			numcars = numcars + 1
			exports.un_anticheat:changeProtectedElementDataEx(value, "owner_last_login", exports.un_datetime:now(), true)
		end
	end
	...
	if job == 0 then
		...
		local jobName = exports["un_jobs"]:getJobTitleFromID(job)
	...
	--CARRIED
	local carried = "Taşınan Ağırlık: "..("%.2f/%.2f" ):format( exports["un_items"]:getCarriedWeight( thePlayer ), exports["un_items"]:getMaxWeight( thePlayer ) ).." kg(s)"
	if isOverlayDisabled then
	...
	if isOverlayDisabled then
		...
		table.insert(info, {""})
		table.insert(info, {"Meslek: "..exports["un_jobs"]:getJobTitleFromID(job)..""})
		table.insert(info, {"Cüzdan: ₺"..exports.un_global:formatMoney(money)})
		table.insert(info, {"Bankadaki Para: ₺"..exports.un_global:formatMoney(bankmoney)})
		table.insert(info, {"Bakiye: "..exports.un_global:formatMoney(currentGC) .. " ₺ "})
