...
local font2 = exports.un_fonts:getFont("RobotoB", 12)
local font3 = exports.un_fonts:getFont("RobotoL", 10.8)
local active = true
local alpha = 220
local badges = {}
local masks = {}
...
function initStuff(res)
	if (res == getThisResource() and getResourceFromName("un_items")) or getResourceName(res) == "un_items" then
		for key, value in pairs(exports['un_items']:getBadges()) do
...
function getPlayerIcons(name, player, forTopHUD, distance, status)
	...
	if getElementData(player, "loggedin") then
		...
		if getElementData(player,"uyk") == 1 and getElementData(player,"uyk_duty") then
			table.insert(icons, "udy_on")
		end

		if getElementData(player,"country") > 0  then
			local cntry = getElementData(player,"country")
			table.insert(icons, cntry)
		end

		if getElementData(player,"rplus") == 1  then
			table.insert(icons, "rp+")
		end
		...
		for key, player in ipairs(getElementsByType("player")) do
			if (isElement(player)) and getElementDimension(player) == dim then
				...
				if (logged == true) then
					...
					local reconx = getElementData(localPlayer, "reconx") and exports.un_integration:isPlayerTrialAdmin(localPlayer)
								...
								if not (collision) or (reconx) then
									...
									if (sx) and (sy) then
										...
										if (sx) and (sy) and oldsy then
											...
											if getElementData(player,"donator") == 1 then
												dxDrawImage(sx-10-(dxGetTextWidth(name, scale, "default-bold")/1.5), sy+35, countryImgW, countryImgH,"components/icons/donatortag1.png")										
											end
											if getElementData(player,"donator") == 2 then
												dxDrawImage(sx-10-(dxGetTextWidth(name, scale, "default-bold")/1.5), sy+35, countryImgW, countryImgH,"components/icons/donatortag2.png")										
											end
											if getElementData(player,"donator") == 3 then
												dxDrawImage(sx-10-(dxGetTextWidth(name, scale, "default-bold")/1.5), sy+35, countryImgW, countryImgH,"components/icons/donatortag3.png")										
											end
	                                        ...
	                                        if getElementData(player, "hud:minimized") then
                                           		tx, ty, tw, th = sx-offset, sy-13, (sx-offset)+160 / distance, sy+100-44 / distance							
                                                dxDrawText("[ AFK ]", tx+1, ty, tw+1, th, tocolor(0, 0, 0, 255), scale, font, "center", "center", false, false, false, false, false)
                                                dxDrawText("[ AFK ]", tx-1, ty, tw-1, th, tocolor(0, 0, 0, 255), scale, font, "center", "center", false, false, false, false, false)
                                                dxDrawText("[ AFK ]", tx, ty+1, tw, th+1, tocolor(0, 0, 0, 255), scale, font, "center", "center", false, false, false, false, false)
                                                dxDrawText("[ AFK ]", tx, ty-1, tw, th-1, tocolor(0, 0, 0, 255), scale, font, "center", "center", false, false, false, false, false)												
                                                dxDrawText("[ AFK ]", sx-offset, sy-13, (sx-offset)+160 / distance, sy+100-44 / distance, tocolor(200, 60, 60, 255), scale, font, "center", "center", false, false, false, false, false)												
	                                        end
											...
											dxDrawText(name, tx+1, ty, tw+1, th, tocolor(0, 0, 0, 255), 0.9, font3, "center", "center", false, false, false, true, false)
											dxDrawText(name, tx-1, ty, tw-1, th, tocolor(0, 0, 0, 255), 0.9, font3, "center", "center", false, false, false, true, false)
											dxDrawText(name, tx, ty+1, tw, th+1, tocolor(0, 0, 0, 255), 0.9, font3, "center", "center", false, false, false, true, false)
											dxDrawText(name, tx, ty-1, tw, th-1, tocolor(0, 0, 0, 255), 0.9, font3, "center", "center", false, false, false, true, false)
											dxDrawText(name, tx, ty, tw, th, tocolor(r, g, b, alpha), 0.9, font3, "center", "center", false, false, false, true, false)
