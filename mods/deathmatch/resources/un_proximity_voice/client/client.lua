    ...
    function()
        ...
        if source:getData("loggedin") == 1 and source:getData("voicemute") then
            if source == localPlayer then
                ...
                source:setData("ses_konusma", false)
            end
            cancelEvent()
        return end
       -- if source:getData("loggedin") == 1 and source:getData("level") == 1 then
            --if source == localPlayer then
               -- outputChatBox("[-]#ffffff 1 seviye oyuncular sesli konuşmadan yararlanamaz.", 255, 79, 79, true)
               -- source:setData("ses_konusma", false)
           -- end
           -- cancelEvent()
       -- return end
