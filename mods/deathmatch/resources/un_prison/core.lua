...
sPrison = {
	...
	sPrison = instance;
		if player:getData('faction') == 1 or player:getData('faction') == 78 then
			if targetPlayerName and tonumber(time) and ... then
				local targetPlayer, targetPlayerName = exports.un_global:findPlayerByPartialNick(player, targetPlayerName)
	...
	_timer = function()
		for index, value in ipairs(exports.un_pool:getPoolElementsByType("player")) do
