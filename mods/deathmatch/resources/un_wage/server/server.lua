...
function timer()
 for index, player in ipairs(exports.un_pool:getPoolElementsByType("player")) do
  ...
  setElementData(player, "minutesPlayed", tonumber(getElementData(player, "minutesPlayed")) + 1)
   if tonumber(getElementData(player, "minutesPlayed")) >= 60 then
	 ...
	 if factionID == 1 or factionID == 2 or factionID == 3 or factionID == 20 or factionID == 78 then
	  ...
	  for key, player in ipairs(exports.un_pool:getPoolElementsByType("team")) do
      ...
      elseif (getElementData(player, "vipver") == 4) then
...
local region = createColSphere(400.2314453125, -1526.84765625, 32.2734375, 3)
local pickup = createPickup(400.2314453125, -1526.84765625, 32.2734375, 3, 1274)
 ...
 if getElementData(source, 'loggedin') == 1 then
  if isElementWithinColShape(source, region) then
   if getElementData(source, 'box') > 0 then
	...
	exports.un_global:giveMoney(source, pay)
